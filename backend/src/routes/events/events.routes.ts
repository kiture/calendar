...
const eventController = {
    ...
    getEventById: async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    updateEvent: async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        ...
        const { title, start_time, place, description, is_ai_suggestion } = req.body;
        ...
        try {
            ...
            if (is_ai_suggestion !== undefined) updates.is_ai_suggestion = is_ai_suggestion;
            ...
            if (result.rowCount === 0) {
                ...
                res.status(404).json({ message: 'Event not found or update not allowed' });
                return;
            }
    ...
    deleteEvent: async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            ...
            if (result.rowCount === 0) {
                await client.query('ROLLBACK');
                // RLS policy prevented delete, or event doesn't exist
                res.status(404).json({ message: 'Event not found or delete not allowed' });
                return;
            }
    ...
    joinEvent: async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        const { eventId } = req.params;
        if (!req.user) return next(new Error('Authentication required'));
        const userId = req.user.userId;
        let client;
        ...
        try {
            ...
            if (checkResult && checkResult.rowCount != null && checkResult.rowCount > 0) {
                ...
                const currentResult = await client.query(currentAttendanceQuery, [eventId, userId]);
            ...
            if (result.rows.length > 0) {
                ...
                throw new Error('Failed to join event. Check if the event exists and you are a member of its group.');
            }

        } catch (error) {
            if (client) try { await client.query('ROLLBACK'); } catch (e) { console.error('Rollback failed', e); }
            if (error instanceof Error && 'code' in error && (error.code === '23503' || error.code === '44000')) { // FK violation or check violation
                 next(new Error('Failed to join event: Event not found or access denied.'));
             } else {
                 next(error);
             }
        } finally {
    ...
    leaveEvent: async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        const { eventId } = req.params;
        if (!req.user) return next(new Error('Authentication required'));
        const userId = req.user.userId;
        let client;
        ...
        try {
            ...
            // RLS policy ensures user can only delete their own attendance
            const deleteQuery = `DELETE FROM event_attendance WHERE event_id = $1 AND user_id = $2`;
            const result = await client.query(deleteQuery, [eventId, userId]);

            if (result.rowCount === 0) {
                await client.query('ROLLBACK');
                // Attendance record not found (or RLS prevented)
                res.status(404).json({ message: 'Attendance record not found or delete not allowed' });
                return;
            }
    ...
    listAttendees: async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        const { eventId } = req.params;
        ...
        try {
            ...
            // RLS policy on SELECT should ensure user is member of the event's group
            const attendeesQuery = `
                SELECT u.user_id, u.login, u.first_name, u.last_name, ea.joined_at
...
const validateUpdateEvent = [
    ...
    body('start_time').optional().isISO8601().withMessage('Valid start_time (ISO8601 format) is required'),
...
router.get('/:eventId', validateEventId, handleValidationErrors, eventController.getEventById);

// PUT /events/:eventId - Update event details
router.put('/:eventId', validateUpdateEvent, handleValidationErrors, eventController.updateEvent);

// DELETE /events/:eventId - Delete event
router.delete('/:eventId', validateEventId, handleValidationErrors, eventController.deleteEvent);

// --- Event Attendance Routes (/events/:eventId/attendance | /events/:eventId/attendees) ---

// POST /events/:eventId/attendance - Join event
router.post('/:eventId/attendance', validateEventId, handleValidationErrors, eventController.joinEvent);

// DELETE /events/:eventId/attendance - Leave event
router.delete('/:eventId/attendance', validateEventId, handleValidationErrors, eventController.leaveEvent);

// GET /events/:eventId/attendees - List attendees
router.get('/:eventId/attendees', validateListAttendees, handleValidationErrors, eventController.listAttendees);
