...
const aiController = {
    ...
    getSuggestions: async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
...
const validateGetSuggestions = [
    queryValidator('startDate').isISO8601().withMessage('Valid startDate (ISO8601 format) is required'),
    queryValidator('endDate').isISO8601().withMessage('Valid endDate (ISO8601 format) is required'),
    queryValidator('location').notEmpty().withMessage('Location query parameter is required'),
...
// GET /ai/event-suggestions
router.get('/event-suggestions', validateGetSuggestions, handleValidationErrors, aiController.getSuggestions);
