...
const adminGroupController = {
  ...
  updateGroup: async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const check = await query('SELECT 1 FROM groups WHERE group_id = $1', [
        groupId,
      ]);
      if (check.rowCount === 0) {
        res.status(404).json({ message: 'Group not found' });
  ...
  deleteGroup: async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const result = await query('DELETE FROM groups WHERE group_id = $1', [
        groupId,
      ]);
      if (result.rowCount === 0) {
        res.status(404).json({ message: 'Group not found' });
...
const adminMembershipController = {
  addMember: async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const userExists = await query('SELECT 1 FROM users WHERE user_id = $1', [
        user_id,
      ]);
      ...
      if (
        membershipExists &&
        typeof membershipExists.rowCount === 'number' &&
        membershipExists.rowCount > 0
      ) {
        res
          .status(409)
  ...
  removeMember: async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (result.rowCount === 0) {
        ...
        res.status(404).json({
          message: 'Membership not found (or group/user does not exist)',
        });
        return;
      }
...
router.post(
  ...
  validateAddMember,
  handleValidationErrors as RequestHandler,
  adminMembershipController.addMember
);
router.get(
  '/:groupId/members',
  validateListMembers,
  handleValidationErrors as RequestHandler,
  adminMembershipController.listMembers
);
router.delete(
  '/:groupId/members/:userId',
  validateMemberParams,
  handleValidationErrors as RequestHandler,
  adminMembershipController.removeMember
);
