...
- **UX/Dostępność/Bezpieczeństwo:** Prosty formularz, czytelna lista wyników. Przycisk "Dodaj do Kalendarza" tworzy wydarzenie w _aktualnie wybranej grupie_ (z Redux) i ustawia flagę `is_ai_suggestion`. Obsługa stanów ładowania i błędów podczas komunikacji z API AI. Endpoint API: `GET /ai/event-suggestions`, `POST /groups/{activeGroupId}/events`.
